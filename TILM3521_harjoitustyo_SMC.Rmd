...
Ongelmassa tiedämme, miten havaitut muuttujat kytkeytyvät "piilossa oleviin" muuttujiin sekä osaamme sanoa jotain tilamuuttujien todennäköisyyksistä. Oletamme, että piilossa oleville tiloille $X_n$ pätee Markov-ominaisuus. Toisin sanoen kutakin hetkeä seuraava tila $x_{n+1}$ riippuu menneistä tiloista $x_{1:n}$ ainoastaan tilan $x_n$ välityksellä. Oletamme myös, että havaittu tila $y_n$ riippuu tiloista $X_{n}$ ainoastaan jonkin $x_k$:n funktion kautta. Kun aika-avaruus on diskreetti ja merkitsemme piilossa olevan prosessin tilaa ajanhetkellä $k$ $x_k$ ja havaittua prosessia $y_k$, meillä on siis olemassa mallit
...
Joitakin suodinongelman rekursiivisia Monte Carlo -ratkaisuja löytyy 50-70-luvuilta, erityisesti säätöteoriaan piiristä. Ensimmäisen epälineaarisen suodinongelman Bayesilaisen, numeeriseen Monte Carlo -estimointiin perustuvan ratkaisun esittivät kuitenkin Gordon, Salmond ja Smith artikkelissaan "Novel approach to nonlinear/non-Gaussian Bayesian state estimation" (1993). Gordonin, Salmondin ja Smithin ratkaisu eroaa notaatioltaan hieman tässä tutkielmassa esitetystä, mutta on olennaisesti sama. Gordonin, Salmondin ja Smithin artikkelissa ratkaisu kulki nimellä "bootstrap filter", saapasremmisuodin. Suurin ero tämän tutkielman SMC-algoritmin sekä artikkelin SMC-algoritmin välillä on uudelleenotannassa, joka alkuperäisessä artikkelissa suoritetaan jokaisella algoritmin iteraatiolla. LASKENTATEHO.
