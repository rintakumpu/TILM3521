...
# And plot
for(i in 1:nrow(locators_xy)) {
  site_map <- site_map + annotate("point", x=locators_xy[i,"x"], y=locators_xy[i,"y"], color="cyan", size=5) + annotate("text", x=locators_xy[i,"x"], y=locators_xy[i,"y"], label=locators_xy[i,"id"], size=3)
}
...
Paikaunnusesimerkissä lähettimenä toimi 25 Bluetooth-paikannustagista koostuva Walkbase Foculator -testilaite (kuva \ref{fig:foculator}), vastaanottimena puolestaan toimistoympäristöön asennetut seitsemän Walkbase XR-2 -vastaanotinta (kuva \ref{fig:xr2}). Jokainen vastaanotin sisältää kahdeksan antennia, XXX havaintojen perusteella vastaanottimet laskevat XXX tulokulman. XXX. Esteettömässä ympäristö paikannus XXX suodatinta ei tarvita. Toimistoympäristö on kuitenkin haastava, sillä erityisesti nä XXX mikä tarjoaa motivaation SMC-menetelmien käyttämiseen.

Data on kuvattu tarkemmin alaluvussa 4.2. Koeympäristön kartta on kuvattuna kaaviossa 1.
...
Toimistympäristön KARTTA, jossa sensorit.

```{r eval=F}
site_map
```

TRAJECTORY. jotka MITÄ. 
